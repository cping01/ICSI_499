package com.damc.driver_action.data.local.DAO
...
import com.damc.driver_action.domain.models.DBTrip
import com.damc.driver_action.domain.models.DBTripmetrics

@Dao
interface TripDao {

    @Insert
    suspend fun insertTripSegment(segment: TripDataclasses.TripSegment)

    @Insert
    suspend fun insertTripMetrics(metrics: DBTripmetrics)

    @Insert
    suspend fun insertTrip(trip: DBTrip)
    ...
    @Query("SELECT tripID FROM trips WHERE endTime IS NULL ORDER BY startTime DESC LIMIT 1")
    suspend fun getActiveTripID(): Int?
